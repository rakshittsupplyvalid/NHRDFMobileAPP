...
const InspectionScreen = () => {
  ...
  const fetchSeedData = async () => {
    ...
    } catch (error) {import React, { useState, useCallback, useEffect } from "react";
import {
  View,
  Text,
  ScrollView,
...
const InspectionScreen = () => {
  ...
  // Count and Offtype rows state
  const [countOfftypeRows, setCountOfftypeRows] = useState<Array<{ count: string; offtype: string }>>([
    { count: "", offtype: "" },
    { count: "", offtype: "" },
  ]);
  ...
  // Add Count and Offtype Row
  const handleAddCountOfftypeRow = () => {
    if (countOfftypeRows.length < MAX_ROWS) {
      setCountOfftypeRows([...countOfftypeRows, { count: "", offtype: "" }]);
    }
  };

  // Handle Count and Offtype field changes
  const handleCountOfftypeChange = (index: number, field: 'count' | 'offtype', value: string) => {
    const updated = [...countOfftypeRows];
    updated[index][field] = value;
    setCountOfftypeRows(updated);
  };
  ...
  const validateField = (field: string, value: any) => {
    ...
    if (formData.IsThisFinalReport && field === "EstimatedSeedYield") {
      if (!value || value.trim() === "") {
        error = "Estimated Yield is required when Final Report is Yes";
      }
    }
  ...
  const handleChange = (field: string, value: any) => {
    ...
    if (field === "IsThisFinalReport") {
      if (value === true) {
        validateField("EstimatedSeedYield", formData.EstimatedSeedYield);
      } else {
        setErrors((prev: any) => ({ ...prev, EstimatedSeedYield: "" }));
      }
    }
  };
    ...
    >
      ...
      >
        <View style={styles.container}>
          ...
          {/* Count and Offtype Card - NEW SECTION */}
          <Card style={styles.card}>
            <View style={styles.header}>
              <MaterialCommunityIcons name="format-list-checks" size={30} color="#2E7D32" />
              <Text style={styles.headerTitle}>Count & Offtype</Text>
            </View>

            <Card.Content style={styles.content}>
              <Text style={styles.sectionDescription}>
                Enter Count and Number of Plant of Offtypes for each row
              </Text>

              {countOfftypeRows.map((row, index) => (
                <View key={index} style={styles.rowContainer}>
                  <View style={styles.rowNumber}>
                    <Text style={styles.rowNumberText}>{index + 1}</Text>
                  </View>
                  <View style={styles.countOfftypeRow}>
                    <View style={styles.halfInput}>
                      ...
                      <TextInput
                        ...
                        keyboardType="numeric"
                        value={row.count}
                        onChangeText={(value) => handleCountOfftypeChange(index, 'count', value)}
                      />
                    </View>

                    <View style={styles.halfInput}>
                      <Text style={styles.label}>Number of Plant of Offtypes</Text>
                      <TextInput
                        style={styles.input}
                        ...
                        keyboardType="numeric"
                        value={row.offtype}
                        onChangeText={(value) => handleCountOfftypeChange(index, 'offtype', value)}
                      />
                    </View>
                  </View>
                </View>
              ...
              <View style={styles.buttonWrap}>
                <Button
                  mode="outlined"
                  onPress={handleAddCountOfftypeRow}
                  disabled={countOfftypeRows.length >= MAX_ROWS}
              ...
              {countOfftypeRows.length >= MAX_ROWS && (
                <Text style={styles.warning}>Maximum 10 entries allowed!</Text>
              )}
            </Card.Content>
          </Card>
...
export default InspectionScreen;
  ...
  const handleChange = (field: string, value: any) => {
    ...
    // 🔥 When FinalReport is changed
    if (field === "IsThisFinalReport") {
      if (value === true) {
        validateField("EstimatedSeedYield", formData.EstimatedSeedYield);
      } else {
        // Clear estimated yield error if switched to NO
        setErrors((prev: any) => ({ ...prev, EstimatedSeedYield: "" }));
      }
    }
